...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not seller_receipt_no or not seller_phno or not customer_phno or not items:
        return func.HttpResponse(
            "Please provide seller_receipt_no, seller_phno, customer_phno, and items.",
            status_code=400
        )
    ...
    for item in items:
        ...
        if not item_name or not quantity or not total_price or not date_of_purchase:
            return func.HttpResponse(
                "Please provide all the required input parameters for each item.",
                status_code=400
            )
    ...
    return func.HttpResponse("Receipt details stored successfully.", status_code=200)
